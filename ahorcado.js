...
// usamos extra para las vocales con tilde
  ...
  if (palabraSecreta.includes(letra) || palabraSecreta.includes(extra)) {   
    ...
    if (errores === 6) {
      ...
      document.getElementById("puntuacion3").value=parseInt(document.getElementById("puntuacion3").value)-20;
      desactivarInput();
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Escucha el evento introducir una letra ********
document.getElementById("letter-input").addEventListener("keyup", function() {
    const letra = document.getElementById("letter-input").value.toLowerCase();
    if (letra) { //&& letra.match(/[a-z]/i)) {
      document.getElementById("letter-input").value = "";
      actualizarEstadoJuego(letra);
